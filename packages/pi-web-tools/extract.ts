...
// node-html-markdown replaces turndown: it preserves tables (turndown drops them
// without the gfm plugin) and over-escapes far less — both better for LLM-fed
// extraction — while dropping the ~9MB @mixmark-io/domino DOM that turndown bundled.
const nhm = new NodeHtmlMarkdown({ codeFence: "```", bulletMarker: "-" });
