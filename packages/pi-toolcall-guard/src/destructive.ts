...
// Destructive-bash analysis, merged in from the former standalone bash-guard
// extension. Pure logic (no pi imports) so it is unit-testable; the index.ts
// tool_call handler decides what to do with a Risk (hard-block in headless
// subagents, interactive ctx.ui.confirm in the main session).
...
/** Headless (subagent) hard-block: returns the reason if a catastrophic pattern matches, else null. */
export function headlessBlockReason(command: string): string | null {
