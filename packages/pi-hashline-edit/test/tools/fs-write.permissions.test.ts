...
// POSIX-only: asserts the tmp path (forward-slash regex) and 0o600 create mode.
// Windows uses `\` separators and has no POSIX file modes, so skip there.
describe.skipIf(process.platform === "win32")("writeFileAtomically permissions", () => {
	beforeEach(() => {
		vi.clearAllMocks();
