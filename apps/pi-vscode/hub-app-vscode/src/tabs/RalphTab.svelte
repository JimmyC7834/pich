...
{:else}
  <vscode-textfield class="search" placeholder="Filter projects…"
    oninput={(e: any) => (q = e.target.value.toLowerCase())}></vscode-textfield>
  {#each shown as p (p.id)}
    <vscode-collapsible class="proj" title={`${p.name}  ·  ${p.done.length}/${count(p)} done`}>
      <div class="cols">
...
<style>
  ...
  .proj{display:block;margin-bottom:8px}
  .n{font-size:.8em;opacity:.6;font-weight:normal}
  .cols{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;padding:8px}
